...
/*
  Axiomatic and properties related to the occurence used of a specific
  element in an array.
...
/*@ // Validity of a MEMB.
  ...
  @   \valid(m) && \valid(m->used) && \valid((char *)m->mem)
...
/*@ // Helper predicates. For readability.
  ...
  @   _memb_has(m, ptr) && m->used[_memb_index(m, ptr)] != 0;
  @*/

/*@ // Couting free elements.
  @
  @ logic integer _memb_numfree(struct memb *m) =
  @   occ_a(0, m->used, 0, m->num);
  @
  @ predicate _memb_empty(struct memb *m) =
  @   \forall integer i; 0 <= i < m->num ==> m->used[i] == 0;
  @
